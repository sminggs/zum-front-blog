...
import { GET as getMediaContentsCount } from './[mediaIdx]';
...
test('GET', async () => {
  const result = await getMediaContentsCount({ mediaIdx: 42 });
  expect(result).toHaveProperty('countItem');
});

test('GET error', () => {
  server.use(rest.get('*', sendServerError('C001')));

  expect(getMediaContentsCount({ mediaIdx: 42 })).rejects.toThrowError('503');
});
